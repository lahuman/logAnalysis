...
2026-09-09 개선: 깊은 원인 보존, 메소드 전체 분석, 높음·중간·낮음 분류,
한글 리포트와 세 줄 요약을 적용했습니다. [작업 기록](docs/ANALYSIS_IMPROVEMENTS.md)과
[한글 리포트 예시](docs/examples/error-priority-report.md)에서 확인할 수 있습니다.
...
| 깊은 원인·메소드 전체·오류 수준·한글 요약 개선 | [오류 분석 개선 작업 기록](docs/ANALYSIS_IMPROVEMENTS.md) |
...
버전 `0.5.0`. 2026-09-09 기준 파일·ES·Oracle 입력, Java 파싱, Git 소스·변경 이력 조회,
LLM 응답 검증, 중복 분석 캐시, 재시도·중단 복구, Markdown 출력과 systemd 설정을 구현했습니다.
실행 오류에는 파일·함수·줄 번호와 원인 예외 체인을 남기며 개별 분석 실패도 이벤트 ID로 추적합니다.

- Windows / Python 3.11.8: **193개 중 187 통과, 6개 조건부 제외**. compileall·pip check·diff check 통과
- 깊은 원인·메소드 전체 분석, 오류 수준 3단계, 한글 리포트와 실행 종료 세 줄 요약
...
- 이번 분석·리포트 변경을 포함한 배포 압축파일 재빌드·실서버 연결 및 실제 모델 품질 검증은 별도 수행 필요
